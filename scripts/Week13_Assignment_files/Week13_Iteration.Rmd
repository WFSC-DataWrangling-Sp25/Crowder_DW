...
#for each volume in a vector, calculate a mass
...
# for loop to calculate masses
for(i in 1:length(volumes)) {
  masses[i] <- as[i] * volumes[i]^bs[i]
}
...
for (i in 1:length(volumes)) {
  masses[i] <- est_mass_max(volumes[i], as[i], bs[i])
}
```
...
```{r}
#option 1: subset by both row and column with []
for (i in 1:nrow(data)){
  masses[i] <- est_mass_max(volume = data[i, "volume"], 
                            a = data[i, "a"],
                            b = data[i, "b"])
}

#option 2: ref specific columns with $ then subset
for (i in 1:nrow(data)){
  masses[i] <- est_mass_max(volume = data$volume[i], 
                            a = data$a[i],
                            b = data$b[i])
}
```
...
unzip(zipfile = "data_raw/locations.zip", #path and name of folder
      exdir = "data_raw/Week13_Assignment_files")
```
...
```{r}
list.files()
list.files(path = "../Week13_Assignment_files/")
```
...
```{r}
data_files <- list.files(path = "../Week13_Assignment_files/", 
           pattern = ".txt")
data_files
```
...
for (i in 1:num_files){
  ...
  #read in the data
  data <- read.csv(paste0("../Week13_Assignment_files/", filename)) #count the number of rows
  count <- nrow(data)
  #save results
  results[i] <- count
}
...
```{r}
results <- data.frame(file_name = character(num_files),
                      count = integer(num_files),
                      min_lat = numeric(num_files))
```
...
for (i in 1:num_files){
  #specify file
  filename <- data_files[i]
  data <- read.csv(paste0("../Week13_Assignment_files/", filename)) 
  
  #adding info to the results dataframe
